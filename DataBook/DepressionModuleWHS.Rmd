...
whs_depr_data_dir <- here::here("DataBook", "Modules", "DepressionModuleWHS")

# list all files in the directory
# fs::dir_ls(whs_depr_data_dir)

# only R files
r_files <- fs::dir_ls(whs_depr_data_dir, regexp = "\\.R$") 
# r_files
...
whs_depression_c_m_w_b <- join_m_w_b(whs_depression_c, 1, whs_depression_c_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
whs_depression_c_u_r <- join_u_r(whs_depression_c, 1, whs_depression_c_col_names)
```
...
cbind(whs_depression_c_m_w_b$m, select(whs_depression_c_m_w_b$w, -agerange), select(whs_depression_c_m_w_b$b, -agerange)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Prevalence of depressive symptoms using established WHS algorithm", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Prevalence of depressive symptoms using established WHS algorithm by type of settlement", .colspan_val = 10)
```
...
whs_depression_c$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = whs_depression_c_reg_col_names) %>% 
  final_m_w_b_tbl(.s_amount = 1, .s_val = "Both sexes", .title = "Prevalence of depressive symptoms using established WHS algorithm by region", .colspan_val = 4) 
```
...
# men, women, both sexes
da1_d_whs_m_w_b <- join_m_w_b(da1_d_whs, 1, da1_d_whs_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
da1_d_whs_u_r <- join_u_r(da1_d_whs, 1, da1_d_whs_col_names)
```
...
cbind(da1_d_whs_m_w_b$m, select(da1_d_whs_m_w_b$w, -agerange), select(da1_d_whs_m_w_b$b, -agerange)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Told by a doctor or health worker that have a depression (from those that are depressed based on WHS algorithm)", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Told by a doctor or health worker that have a depression (from those that are depressed based on WHS algorithm) by type of settlement", .colspan_val = 10)
```
...
da1_d_whs$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = da1_d_whs_reg_col_names) %>% 
  final_m_w_b_tbl(.s_amount = 1, .s_val = "Both sexes", .title = "Told by a doctor or health worker that have a depression (from those that are depressed based on WHS algorithm) by region", .colspan_val = 4) 
```
...
# men, women, both sexes
da1_d_all_m_w_b <- join_m_w_b(da1_d_all, 1, da1_d_all_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
da1_d_all_u_r <- join_u_r(da1_d_all, 1, da1_d_all_col_names)
```
...
cbind(da1_d_all_m_w_b$m, select(da1_d_all_m_w_b$w, -agerange), select(da1_d_all_m_w_b$b, -agerange)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Told by a doctor or health worker that have a depression", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Told by a doctor or health worker that have a depression by type of settlement", .colspan_val = 10)
```
...
da1_d_all$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = da1_d_all_reg_col_names) %>% 
  final_m_w_b_tbl(.s_amount = 1, .s_val = "Both sexes", .title = "Told by a doctor or health worker that have a depression by region", .colspan_val = 4) 
```
...
da4_e_all_m_w_b <- join_m_w_b(da4_e_all, 1, da4_e_da5_f_all_col_names)
# 2 - Have been taking medications or other treatment for depression in the last 2 weeks
da5_f_all_m_w_b <- join_m_w_b(da5_f_all, 1, da4_e_da5_f_all_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
# 1
da4_e_all_u_r <- join_u_r(da4_e_all, 1, da4_e_da5_f_all_col_names)
# 2
da5_f_all_u_r <- join_u_r(da5_f_all, 1, da4_e_da5_f_all_col_names)
```
...
cbind(da4_e_all_m_w_b$m, select(da4_e_all_m_w_b$w, -agerange), select(da4_e_all_m_w_b$b, -agerange)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Have been taking medications or other treatment for depression in the last 12 months", .colspan_val = 10)
```
...
cbind(da5_f_all_m_w_b$m, select(da5_f_all_m_w_b$w, -agerange), select(da5_f_all_m_w_b$b, -agerange)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Have been taking medications or other treatment for depression in the last 2 weeks", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Have been taking medications or other treatment for depression in the last 12 months by type of settlement", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Have been taking medications or other treatment for depression in the last 2 weeks by type of settlement", .colspan_val = 10)
```
...
da4_e_all$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = da4_e_da5_f_all_reg_col_names) %>% 
  final_m_w_b_tbl(.s_amount = 1, .s_val = "Both sexes", .title = "Have been taking medications or other treatment for depression in the last 12 months by region", .colspan_val = 4) 
```
...
da5_f_all$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = da4_e_da5_f_all_reg_col_names) %>% 
  final_m_w_b_tbl(.s_amount = 1, .s_val = "Both sexes", .title = "Have been taking medications or other treatment for depression in the last 2 weeks by region", .colspan_val = 4) 
```
...
<!-- Description: Percentage of population who have thought of death, or wished they were dead, during the last 12 months. -->
...
<!-- ```{r da22_g_all_tbls} -->
<!-- # column names to use with huxtable -->
<!-- da22_g_all_col_names <- c("Age Group (years)", "n", "% Thought", "95% CI") -->
<!-- da22_g_all_reg_col_names <- c("Region", "n", "% Thought", "95% CI") -->

<!-- # men, women, both sexes -->
<!-- da22_g_all_m_w_b <- join_m_w_b(da22_g_all, 1, da22_g_all_col_names) -->
<!-- ``` -->
<!-- ```{r, eval=.GlobalEnv$.u_r_reg==TRUE} -->
<!-- # urban, rural -->
<!-- da22_g_all_u_r <- join_u_r(da22_g_all, 1, da22_g_all_col_names) -->
<!-- ``` -->

<!-- <br> -->

<!-- ```{r} -->
<!-- cbind(da22_g_all_m_w_b$m, select(da22_g_all_m_w_b$w, -agerange), select(da22_g_all_m_w_b$b, -agerange)) %>%  -->
<!--   final_m_w_b_tbl(.s_amount = 3, .title = "Thought of death, or wished they were dead, during the last 12 months", .colspan_val = 10) -->
<!-- ``` -->

<!-- <br> -->

<!-- ```{r, eval=.GlobalEnv$.u_r_reg==TRUE} -->
<!-- tbl_m_u_r <- rbind(da22_g_all_u_r$m_u, da22_g_all_u_r$m_r) -->
<!-- tbl_w_u_r <- rbind(da22_g_all_u_r$w_u, da22_g_all_u_r$w_r) -->
<!-- tbl_b_u_r <- rbind(da22_g_all_u_r$b_u, da22_g_all_u_r$b_r) -->

<!-- cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>%  -->
<!--   final_u_r_tbl(.s_amount = 3, .title = "Thought of death, or wished they were dead, during the last 12 months by type of settlement", .colspan_val = 10) -->
<!-- ``` -->

<!-- <br> -->

<!-- ```{r, eval=.GlobalEnv$.u_r_reg==TRUE} -->
<!-- da22_g_all$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = da22_g_all_reg_col_names) %>%  -->
<!--   final_m_w_b_tbl(.s_amount = 1, .s_val = "Both sexes", .title = "Thought of death, or wished they were dead, during the last 12 months by region", .colspan_val = 4)  -->
<!-- ``` -->
...
<!-- - Questions used: DA6, DA7, DA8, DA22 -->
<!-- - R script name: WHSdepression -->
...
# men, women, both sexes
da22_g_whs_m_w_b <- join_m_w_b(da22_g_whs, 1, da22_g_whs_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
da22_g_whs_u_r <- join_u_r(da22_g_whs, 1, da22_g_whs_col_names)
```
...
cbind(da22_g_whs_m_w_b$m, select(da22_g_whs_m_w_b$w, -agerange), select(da22_g_whs_m_w_b$b, -agerange)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Thought of death, or wished they were dead, during the last 12 months (from those that are depressed based on WHS algorithm)", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Thought of death, or wished they were dead, during the last 12 months (from those that are depressed based on WHS algorithm) by type of settlement", .colspan_val = 10)
```
...
da22_g_whs$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = da22_g_whs_reg_col_names) %>% 
  final_m_w_b_tbl(.s_amount = 1, .s_val = "Both sexes", .title = "Thought of death, or wished they were dead, during the last 12 months (from those that are depressed based on WHS algorithm) by region", .colspan_val = 4) 
```
