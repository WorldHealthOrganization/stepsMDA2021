...
cholesterol_data_dir <- here::here("DataBook", "Modules", "HistoryofRaisedTotalCholesterol")

# list all files in the directory
# fs::dir_ls(cholesterol_data_dir)

# only R files
r_files <- fs::dir_ls(cholesterol_data_dir, regexp = "\\.R$") 
# r_files
...
hchol_c_m_w_b <- join_m_w_b(hchol_c, 4, hchol_c_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
hchol_c_u_r <- join_u_r(hchol_c, 4, hchol_c_col_names)
```

<br>

```{r}
hchol_c_m_w_b$m %>% final_m_w_b_tbl(1, "Men", "Total cholesterol measurement and diagnosis", 10)
```

<br>

```{r}
hchol_c_m_w_b$w %>% final_m_w_b_tbl(1, "Women", "Total cholesterol measurement and diagnosis", 10)
```

<br>

```{r}
hchol_c_m_w_b$b %>% final_m_w_b_tbl(1, "Both sexes", "Total cholesterol measurement and diagnosis", 10)
```
...
hchol_c$b_reg %>% unite_ci(.ci_amount = 4) %>% apply_hux(.col_names = hchol_c_reg_col_names) %>% 
  final_m_w_b_tbl(1, "Both sexes", "Total cholesterol measurement and diagnosis by region", 10) 
```
...
# men, women, both sexes
hchol_d_m_w_b <- join_m_w_b(hchol_d, 1, hchol_d_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
hchol_d_u_r <- join_u_r(hchol_d, 1, hchol_d_col_names)
```
...
cbind(hchol_d_m_w_b$m, select(hchol_d_m_w_b$w, -agerange), select(hchol_d_m_w_b$b, -agerange)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Currently taking oral treatment (medication) prescribed for raised total cholesterol among those previously diagnosed", .colspan_val = 10)
```
...
hchol_d$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hchol_d_reg_col_names) %>% 
  final_m_w_b_tbl(1, "Both sexes", "Currently taking oral treatment (medication) prescribed for raised total cholesterol among those previously diagnosed by region", 4)
```
...
# men, women, both sexes
hcholtrad_c_m_w_b <- join_m_w_b(hcholtrad_c, 1, hcholtrad_c_col_names)

# 2 - Currently taking herbal or traditional treatment for raised cholesterol among those previously diagnosed
# men, women, both sexes
hcholtrad_d_m_w_b <- join_m_w_b(hcholtrad_d, 1, hcholtrad_d_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
# 1 - Seen a traditional healer for raised cholesterol among those previously diagnosed
hcholtrad_c_u_r <- join_u_r(hcholtrad_c, 1, hcholtrad_c_col_names)
# 2 - Currently taking herbal or traditional treatment for raised cholesterol among those previously diagnosed
hcholtrad_d_u_r <- join_u_r(hcholtrad_d, 1, hcholtrad_d_col_names)
```
...
```{r}
# 1
cbind(hcholtrad_c_m_w_b$m, select(hcholtrad_c_m_w_b$w, -agerange), select(hcholtrad_c_m_w_b$b, -agerange)) %>% final_m_w_b_tbl(.s_amount = 3, .title = "Seen a traditional healer for raised cholesterol among those previously diagnosed", .colspan_val = 10)
```
...
```{r}
# 2
cbind(hcholtrad_d_m_w_b$m, select(hcholtrad_d_m_w_b$w, -agerange), select(hcholtrad_d_m_w_b$b, -agerange)) %>% final_m_w_b_tbl(.s_amount = 3, .title = "Currently taking herbal or traditional treatment for raised cholesterol among those previously diagnosed", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 1
hcholtrad_c$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hcholtrad_c_reg_col_names) %>% final_m_w_b_tbl(1, "Both sexes", "Seen a traditional healer for raised cholesterol among those previously diagnosed by region", 4)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 2
hcholtrad_d$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hcholtrad_d_reg_col_names) %>% final_m_w_b_tbl(1, "Both sexes", "Currently taking herbal or traditional treatment for raised cholesterol among those previously diagnosed by region", 4)
```
