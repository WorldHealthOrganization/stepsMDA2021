...
cvd_data_dir <- here::here("DataBook", "Modules", "CardiovascularDiseaseRisk")
...
cvdrisk_c_m_w_b <- join_m_w_b(cvdrisk_c, 1, cvdrisk_c_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
cvdrisk_c_u_r <- join_u_r(cvdrisk_c, 1, cvdrisk_c_col_names)
```
...
cbind(cvdrisk_c_m_w_b$m, select(cvdrisk_c_m_w_b$w, -1), select(cvdrisk_c_m_w_b$b, -1)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Percentage of respondents with a 10-year CVD risk ≥30% or with existing CVD", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -1), select(tbl_b_u_r, -1)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Percentage of respondents with a 10-year CVD risk ≥30% or with existing CVD by type of settlement", .colspan_val = 10)
```
...
cvdrisk_c$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = cvdrisk_c_reg_col_names) %>% 
  final_m_w_b_tbl(1, "Both sexes", "Percentage of respondents with a 10-year CVD risk ≥30% or with existing CVD by region", 4)
```
...
# men, women, both sexes
cvdrisk_d_m_w_b <- join_m_w_b(cvdrisk_d, 1, cvdrisk_d_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
cvdrisk_d_u_r <- join_u_r(cvdrisk_d, 1, cvdrisk_d_col_names)
```
...
cbind(cvdrisk_d_m_w_b$m, select(cvdrisk_d_m_w_b$w, -1), select(cvdrisk_d_m_w_b$b, -1)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Percentage of eligible persons receiving drug therapy and counseling to prevent heart attacks and strokes", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -1), select(tbl_b_u_r, -1)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Percentage of eligible persons receiving drug therapy and counseling to prevent heart attacks and strokes by type of settlement", .colspan_val = 10)
```
...
cvdrisk_d$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = cvdrisk_d_reg_col_names) %>% 
  final_m_w_b_tbl(1, "Both sexes", "Percentage of eligible persons receiving drug therapy and counseling to prevent heart attacks and strokes by region", 4)
```
