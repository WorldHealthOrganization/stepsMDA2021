...
bp_data_dir <- here::here("DataBook", "Modules", "HistoryofRaisedBloodPressure")

# list all files in the directory
# fs::dir_ls(bp_data_dir)

# only R files
r_files <- fs::dir_ls(bp_data_dir, regexp = "\\.R$") 
# r_files
...
hbloodpressure_c_m_w_b <- join_m_w_b(hbloodpressure_c, 4, hbloodpressure_c_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
hbloodpressure_c_u_r <- join_u_r(hbloodpressure_c, 4, hbloodpressure_c_col_names)
```

<br>

```{r}
hbloodpressure_c_m_w_b$m %>% final_m_w_b_tbl(1, "Men", "Blood pressure measurement and diagnosis", 10)
```

<br>

```{r}
hbloodpressure_c_m_w_b$w %>% final_m_w_b_tbl(1, "Women", "Blood pressure measurement and diagnosis", 10)
```

<br>

```{r}
hbloodpressure_c_m_w_b$b %>% final_m_w_b_tbl(1, "Both sexes", "Blood pressure measurement and diagnosis", 10)
```
...
rbind(hbloodpressure_c_u_r$m_u, hbloodpressure_c_u_r$m_r) %>% 
  final_u_r_tbl(1, "Men", "Blood pressure measurement and diagnosis by type of settlement", 10)
```
...
rbind(hbloodpressure_c_u_r$w_u, hbloodpressure_c_u_r$w_r) %>% 
  final_u_r_tbl(1, "Women", "Blood pressure measurement and diagnosis by type of settlement", 10)
```
...
rbind(hbloodpressure_c_u_r$b_u, hbloodpressure_c_u_r$b_r) %>% 
  final_u_r_tbl(1, "Both sexes", "Blood pressure measurement and diagnosis by type of settlement", 10)
```
...
hbloodpressure_c$b_reg %>% unite_ci(.ci_amount = 4) %>% apply_hux(.col_names = hbloodpressure_c_reg_col_names) %>% 
  final_m_w_b_tbl(1, "Both sexes", "Blood pressure measurement and diagnosis by region", 10) 
```
...
# men, women, both sexes
hbloodpressure_d_m_w_b <- join_m_w_b(hbloodpressure_d, 1, hbloodpressure_d_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
hbloodpressure_d_u_r <- join_u_r(hbloodpressure_d, 1, hbloodpressure_d_col_names)
```
...
cbind(hbloodpressure_d_m_w_b$m, select(hbloodpressure_d_m_w_b$w, -agerange), select(hbloodpressure_d_m_w_b$b, -agerange)) %>% 
  final_m_w_b_tbl(.s_amount = 3, .title = "Currently taking drugs (medication) for raised blood pressure prescribed by doctor or health worker among those diagnosed", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Currently taking drugs (medication) for raised blood pressure prescribed by doctor or health worker among those diagnosed by type of settlement", .colspan_val = 10)
```
...
hbloodpressure_d$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hbloodpressure_d_reg_col_names) %>% 
  final_m_w_b_tbl(1, "Both sexes", "Currently taking drugs (medication) for raised blood pressure prescribed by doctor or health worker among those diagnosed by region", 4)
```
...
# men, women, both sexes
hraisedbptrad_c_m_w_b <- join_m_w_b(hraisedbptrad_c, 1, hraisedbptrad_c_col_names)

# 2 - Currently taking herbal or traditional remedy for raised blood pressure among those previously diagnosed
# men, women, both sexes
hraisedbptrad_d_m_w_b <- join_m_w_b(hraisedbptrad_d, 1, hraisedbptrad_d_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
# 1 - Seen a traditional healer among those previously diagnosed
hraisedbptrad_c_u_r <- join_u_r(hraisedbptrad_c, 1, hraisedbptrad_c_col_names)
# 2 - Currently taking herbal or traditional remedy for raised blood pressure among those previously diagnosed
hraisedbptrad_d_u_r <- join_u_r(hraisedbptrad_d, 1, hraisedbptrad_d_col_names)
```
...
```{r}
# 1
cbind(hraisedbptrad_c_m_w_b$m, select(hraisedbptrad_c_m_w_b$w, -agerange), select(hraisedbptrad_c_m_w_b$b, -agerange)) %>% final_m_w_b_tbl(.s_amount = 3, .title = "Seen a traditional healer among those previously diagnosed", .colspan_val = 10)
```
...
```{r}
# 2
cbind(hraisedbptrad_d_m_w_b$m, select(hraisedbptrad_d_m_w_b$w, -agerange), select(hraisedbptrad_d_m_w_b$b, -agerange)) %>% final_m_w_b_tbl(.s_amount = 3, .title = "Currently taking herbal or traditional remedy for raised blood pressure among those previously diagnosed", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Seen a traditional healer among those previously diagnosed by type of settlement", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_u_r_tbl(.s_amount = 3, .title = "Currently taking herbal or traditional remedy for raised blood pressure among those previously diagnosed by type of settlement", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 1
hraisedbptrad_c$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hraisedbptrad_c_reg_col_names) %>% final_m_w_b_tbl(1, "Both sexes", "Seen a traditional healer among those previously diagnosed by region", 4)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 2
hraisedbptrad_d$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hraisedbptrad_d_reg_col_names) %>% final_m_w_b_tbl(1, "Both sexes", "Currently taking herbal or traditional remedy for raised blood pressure among those previously diagnosed by region", 4)
```
