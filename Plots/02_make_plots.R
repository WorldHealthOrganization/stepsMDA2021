      ...
      tryCatch(
        {
          p <- ggplot(data = plotvalues, 
            ...
            #labs(color = .labs, shape = if(.ur) "Settlement" else NULL) + # adjust label text. NOTE: {{ }} syntax doesn't work here
      ...
      tryCatch(
        {
          p <- ggplot(data=plotvalues, 
            ...
            xlab(NULL) + # remove x label 
            ylab({{ .ylab }}) + # specify y label
            #labs(color = .labs, shape = if(.ur) "Settlement") + # adjust label text 
            labs(color = if(deparse(substitute(.agerange))=="agerange" | 
                            deparse(substitute(.agerange))=="agerange2") "Age range" else "Region", 
                 shape = if(.ur) "Settlement") + # adjust label text for agerange and ur
            theme_bw() + # use a white background
...
# NOTE: below is obsolete code that was initially written to generate forest plots,
# using ggforestplot package. However, the concept has been since rewritten,
# using ggplot package into a function. Additional parts of code has been updated to 
# include specific indicators for proper plotting and errors have been corrected. 
# The XLSX mapping has been updated accordingly. 
# The code below is kept for referencing and back-up.
